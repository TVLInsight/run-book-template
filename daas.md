...
**Service or system name: DaaS service** 
...
If for any reason either the batch scripts or the daas scripts fail and the daas files are corrupted or not written out for the day, then we can use the following 7 steps to recover.
1. Restore `OrgSeriesDB.sasbSpotlights` from the previous day's backup
2. Pause all consumers on `prod-i360-v2-batch*` and `prod-i360-v2-consumer*` machines
3. If any code needs to be rolled-back then perform the rollback
4. Run `prod-i360-v2-batch*:/i360/bin/nightly-recalc`
5. Run `prod-i360-v2-daas*:/i360/bin/daas-nightly-run.sh`
6. `rsync` all of current day's daas files generated by previous step into the SFTP server
7. Resume all consumers that were paused in step 2
